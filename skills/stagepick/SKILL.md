...
1. **Inspect**: `stagepick list --json` (add `--lines` for human-readable L-numbers).
...
- Part of a hunk, e.g. splitting a replacement's deletion from its addition,
  or one of several adjacent additions → `id@L<numbers>` from `list --json`
  `changedLines[].i`.
...
- **L-numbers are not file line numbers.** `@L` counts changed (+/-) lines in
  ...
  `list --json` (`changedLines[].i`) or `list --lines`. Never guess them.
...
- **Identical edits share one id and stage together.** Check `idCount` in
  `list --json`; when >1 use `path#id` and `@L` to address just one occurrence.
...
```
$ stagepick list --json   # hunk 91ab34cd, changedLines: i=1 del oldParser, i=2 add newParser
$ stagepick stage 91ab34cd@L1   # stage only the deletion
$ git commit -m "refactor: drop old parser call"
$ stagepick list --json         # re-list: the hunk id changed after the split
...
`list --json`, match `changedLines[].text` to your edits, stage your runs by
